...
const TmuxNative = @import("../terminal/tmux_native.zig").TmuxNative;
...
pub fn init(self: *Termio, alloc: Allocator, opts: termio.Options) !void {
    ...
    self.* = .{
        ...
        .tmux_native = null, // Initialize to null
    };
    
    // Initialize native tmux integration after self is fully set up
    {
        log.info("Initializing native tmux integration...", .{});
        self.tmux_native = try TmuxNative.init(alloc, &self.terminal);
        log.info("Native tmux enabled - Ctrl-B commands work!", .{});
    }
...
pub fn deinit(self: *Termio) void {
    // Cleanup Week 7 auto-split first
    if (self.terminal.auto_split) |auto_split| {
        self.alloc.destroy(auto_split);
        self.terminal.auto_split = null;
    }
    
    // Cleanup native tmux integration
...
pub inline fn queueWrite(
    ...
    // Check if native tmux wants to intercept keyboard input
    if (self.tmux_native) |tmux| {
        // Check for Ctrl-B (0x02) or if we're already in tmux mode
        if (data.len > 0) {
            for (data) |byte| {
                if (tmux.processKey(byte) catch false) {
                    // tmux handled the input
                    return;
                }
            }
        }
...
pub fn handleTmuxCommand(self: *Termio, input: []const u8) bool {
    ...
    if (std.mem.startsWith(u8, input, "@tmux ")) {
        ...
        if (self.tmux_native) |tmux| {
            // For now, just log the command since executeCommand doesn't exist
            log.info("tmux command received: {s}", .{cmd});
