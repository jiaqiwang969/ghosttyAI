...
// Complete tty_ctx definition for backend_router
struct tty_ctx {
    int sx, sy;           // Screen dimensions
    int cx, cy;           // Cursor position
    int ocx, ocy;         // Old cursor position
    int xoff, yoff;       // Offsets
    int rupper, rlower;   // Region bounds
    int orupper, orlower; // Old region bounds
    unsigned int num;     // Numeric parameter
    void* ptr;           // Generic pointer
    const void* data;    // Data pointer
    size_t len;          // Data length
    void* cell;          // Grid cell pointer
    void* wp;            // Window pane pointer
};
